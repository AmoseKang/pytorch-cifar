...
from modefied_models.attention_net import Attnet
from utils import progress_bar

from tensorboardX import SummaryWriter
import numpy as np

run_label = 'res18_100_att'
...
def main(L1_penalty=0.1, LD=5e-4):
    global best_acc
    ...
    att_optimizer = optim.SGD(att.parameters(), lr=1e-3,
                              momentum=0.9, weight_decay=LD)
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            weighted_loss = (loss * weight.squeeze()).sum()
            # weighted_loss = (loss * (loss != loss.min()).float()).mean() # basic self-paced


            # print(L1_penalty * (weight * weight).sum())
            # loss = weighted_loss + L1_penalty * (weight * weight).sum()
            loss = weighted_loss
            ...
            progress_bar(batch_idx, len(trainloader), 'Loss: %.3f/%.3f | Acc: %.3f%% (%d/%d)'
                         % (train_loss/(batch_idx+1), loss.item(), 100.*correct/total, correct, total))
...
#         0.01 0.001  25.97
#         0.05 0.01   25.39
for L1_penalty in [2e3]:
    for ld in [1e-1,1e-2,1e-3]:
        main(L1_penalty, ld)
        acc = best_acc
        print('%s %s acc: %s' % (L1_penalty, ld, acc))
        if acc > best:
            para = (L1_penalty, ld, acc)
            best = acc
